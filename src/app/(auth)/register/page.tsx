    ...
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2">
      ...
      <div className="hidden lg:flex relative bg-gray-950 flex-col justify-between p-12">
        <div
          ...
          style={{ backgroundImage: "url('https://images.unsplash.com/photo-1460661419201-fd4cecdf8a8b?w=1200')" }}
        />
        ...
        <div className="relative space-y-4">
          <div className="space-y-3">
            {["Browse 2,400+ original artworks", "Open your own shop for free", "Secure payments via Stripe", "Connect with artists worldwide"].map((item) => (
              <div key={item} className="flex items-center gap-3 text-gray-300">
                <div className="w-5 h-5 bg-rose-600 rounded-full flex items-center justify-center text-white text-xs">✓</div>
                {item}
              </div>
            ))}
          </div>
      ...
      <div className="flex items-center justify-center p-8 bg-white">
        <div className="w-full max-w-md">
          ...
          <p className="text-gray-500 mb-8">Join thousands of artists and collectors on ArtHub</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <Input id="name" type="text" placeholder="Your name" className="mt-1.5 h-11" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required />
            </div>
            <div>
              <Label htmlFor="email">Email</Label>
              <Input id="email" type="email" placeholder="you@example.com" className="mt-1.5 h-11" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required />
            </div>
            <div>
              <Label htmlFor="password">Password</Label>
              <div className="relative mt-1.5">
                <Input id="password" type={showPass ? "text" : "password"} placeholder="Min 8 characters" className="h-11 pr-10" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required minLength={8} />
          ...
          <Button variant="outline" className="w-full h-12 border-gray-200" onClick={() => signIn("google", { callbackUrl: "/" })}>
